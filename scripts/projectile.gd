...
func _setup_sprite() -> void:
	...
	#RenderingServer.canvas_item_set_transform(sprite_rid,Transform2D(0,start_position))
...
func _setup_body() -> void:
	...
	PhysicsServer2D.body_set_state(body_rid,PhysicsServer2D.BODY_STATE_TRANSFORM,Transform2D(0,Vector2.ZERO))
	PhysicsServer2D.body_set_force_integration_callback(body_rid, self._move_body, "_body_moved")
...
func _physics_process(delta: float) -> void: 
	...
	if !ray_result.is_empty() and instance_from_id(ray_result.collider_id) != null:
		...
		elif(collision_flag and instance_from_id(ray_result.collider_id).is_class("StaticBody2D")):
			_remove_this()
