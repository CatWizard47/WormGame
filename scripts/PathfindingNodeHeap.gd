...
func insert(added_node:PathfindingNode,priority:int)->void:
	...
	while index > 0 and heap[(index-1)/2 > heap[index]]: #this should work since dividing int by int should give int as per documentation
		temporary_element = heap[index]
		heap[index] = heap[(index-1)]
		heap[(index-1)] = temporary_element
