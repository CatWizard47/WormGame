...
@export var start_rotation_radian: float = 0 # is offset by -PI/2 for some reason since rot:0deg = right #
...
func _process(_delta: float) -> void: 
	...
	if  abs(global_rotation - desired_rotation) > accuracy_margin_radian: 
		if desired_rotation - global_rotation  > 0 and desired_rotation - global_rotation < PI or desired_rotation - global_rotation < -PI:
